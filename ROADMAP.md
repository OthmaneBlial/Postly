...
- [ ] Effectuer les validations sur les machines cibles non macOS et joindre un rapport par plateforme ; aucune compatibilité Windows/Linux/Intel n'est déduite du build ARM64. Les liens release Linux (ELF x64), Windows (PE32+ x64 via MinGW, avec icône GUI embarquée) et macOS Intel (Mach-O x86_64, CLI rejouée sous Rosetta) passent désormais sur macOS, mais les packaging/runtimes cibles et la machine Intel réelle restent ouverts ; les preuves sont consignées dans [le rapport de plateforme](docs/measurements/2026-09-08-cross-platform.md) et le [modèle de rapport externe](docs/measurements/external-validation-template.md).
