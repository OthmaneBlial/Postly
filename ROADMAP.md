...
- [x] Rejouer l'archive dans un environnement utilisateur isolé sans Cargo ni Node ; version, workspace, API locale, checksums et démarrage GUI passent. Le replay est codifié dans [tools/replay-package.sh](tools/replay-package.sh) (tar macOS) et [tools/replay-package.ps1](tools/replay-package.ps1) (ZIP Windows), puis prouvé dans [la mesure isolée](docs/measurements/2026-09-08-isolated-package.md).
...
- [x] Corriger le packaging pour utiliser les noms exécutables propres à chaque OS et inclure le commit source, la toolchain et la cible dans le manifeste. Le packager accepte aussi une cible explicite (`cargo xtask package --target TRIPLE`) et vérifie les archives cross-compilées sans exécuter les binaires étrangers. Implémenté et testé localement ; [preuve candidat](docs/release-validation-v0.2.0-preview.1.md), sans validation Windows/Linux implicite.
...
- [ ] Effectuer les validations sur les machines cibles non macOS et joindre un rapport par plateforme ; aucune compatibilité Windows/Linux/Intel n'est déduite du build ARM64. Les artefacts cross-compilés Linux (ELF x64), le ZIP Windows x64 reproductible (PE32+ via MinGW, avec icône GUI embarquée) et macOS Intel (Mach-O x86_64, CLI rejouée sous Rosetta) passent désormais par les contrôles locaux, mais les packaging/runtimes cibles et la machine Intel réelle restent ouverts ; les preuves sont consignées dans [le rapport de plateforme](docs/measurements/2026-09-08-cross-platform.md) et le [modèle de rapport externe](docs/measurements/external-validation-template.md).
...
- [ ] Avant toute prochaine publication, télécharger les assets candidats sur une machine propre et rejouer le scénario vidéo ; `v0.2.0-preview.1` a été publiée sans cette preuve indépendante, et l'écart reste explicitement suivi.
