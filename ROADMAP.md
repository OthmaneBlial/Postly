...
- [ ] Effectuer les validations sur les machines cibles non macOS et joindre un rapport par plateforme ; aucune compatibilité Windows/Linux/Intel n'est déduite du build ARM64. La compilation CLI Linux/Windows et le lien release Linux (CLI + GUI ELF x64) passent désormais via Zig sur macOS, mais le packaging/runtimes Linux, le lien Windows (`msvcrt`) et la machine Intel restent ouverts ; les preuves et échecs sont consignés dans [le rapport de plateforme](docs/measurements/2026-09-08-cross-platform.md).
