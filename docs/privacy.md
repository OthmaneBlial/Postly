...
- Imported or legacy environment files can still contain plaintext secret values. They remain supported for migration; use `postly import environment environment.json --secure` for a secure Postman import, or `postly env migrate --key KEY` for an explicit value / `postly env migrate --all` for imported variables marked as secret. The secret marker alone does not encrypt an existing file.
