...
- Linux: a `.tar.gz`, including an installable desktop entry, the canonical
  reverse-DNS SVG icon and `install-linux-desktop.sh` for a rootless per-user
  integration;
...
The Linux helper installs the packaged
`io.github.othmaneblial.postly.svg` into the user's hicolor theme and writes a
launcher with an absolute `Exec` path, so launching from a desktop menu uses
the same icon and binary as the extracted archive. The reverse-DNS name avoids
stale collisions with another `postly` icon already installed on the machine.
It does not require root and never touches a workspace. Validate its shell
syntax with `sh -n tools/install-linux-desktop.sh`; running it is a user-scoped
filesystem change and is intentionally separate from the archive smoke test.
