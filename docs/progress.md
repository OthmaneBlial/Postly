...
- Basic, Bearer, API-key, OAuth 2.0 Client Credentials, explicit Authorization Code + PKCE, Refresh Token and Device Authorization Grant exchanges in the native model, GUI, CLI, Postman import/export and shared HTTP engine, with bounded approval polling, in-memory expiry-aware token caching and local token-exchange integration tests.
...
integration coverage. Browser callback UX for Authorization Code + PKCE and
AWS Signature V4 remain separate planned work.
