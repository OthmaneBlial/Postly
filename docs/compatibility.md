...
| common headers, bodies and auth | working slice; GUI edits raw/JSON/GraphQL, URL-encoded, multipart and binary-file bodies; OAuth 2.0 Client Credentials, explicit Authorization Code + PKCE token exchange, Refresh Token exchange and Device Authorization Grant polling supported | model/import/export coverage, GUI round-trip tests and local token-exchange/cache integration tests |
