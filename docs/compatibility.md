...
| WebSocket | CLI and native GUI bidirectional `ws://`/`wss://` slice with interactive text console, named saved text presets, header/query auth, bounded reconnects and cancellation | local echo/reconnect integration tests, request-builder coverage, local preset persistence and GUI worker/cancellation tests |
