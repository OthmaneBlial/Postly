...
| HTTPS certificates | core and CLI PEM CA/client-identity slice plus password-protected PKCS#12 `.p12`/`.pfx` identities; native GUI HTTP/SSE Transport settings with session-only passphrase; per-domain association pending | local HTTPS CA, mutual-TLS PEM and PKCS#12 identity parsing tests, GUI settings persistence and file/format diagnostics |
