...
| Explicit response assertions | working core/runner and native GUI editor slice | exact/ranged status, present/absent/equal/containing headers and cookies, valid JSON body, body text, response-time threshold, JSON Pointer presence/absence/equality/deep-inclusion/type, plus a bounded JSON Schema subset with structure/composition/bounds, tuple and `prefixItems` arrays, `contains` cardinality, string patterns and dynamic property validation, runner integration and GUI round-trip coverage |
