...
| common headers, bodies and auth | working slice; GUI edits raw/JSON/GraphQL, URL-encoded, multipart and binary-file bodies; OAuth 2.0 Client Credentials, Authorization Code + PKCE explicit exchange plus loopback browser callback, Refresh Token exchange and Device Authorization Grant polling supported | model/import/export coverage, GUI round-trip tests and local token-exchange/cache/browser-callback integration tests |
| HTTP(S)/SOCKS proxy routing | working HTTP/SSE slice plus CLI/GUI WebSocket and gRPC HTTP CONNECT routing; explicit CLI/GUI proxy, bypass list and env/system proxy support; SOCKS WebSocket remains pending | local forwarding, direct bypass, WebSocket CONNECT relay, gRPC HTTP/2 CONNECT relay, SOCKS URL construction and invalid-proxy tests |
