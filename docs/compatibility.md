...
| common headers, bodies and auth | working slice; GUI edits raw/JSON/GraphQL, URL-encoded, multipart and binary-file bodies; OAuth 2.0 Client Credentials, explicit Authorization Code + PKCE token exchange and Refresh Token exchange supported; Device Code requires review | model/import/export coverage, GUI round-trip tests and local token-exchange/cache integration tests |
| HTTP(S)/SOCKS proxy routing | working HTTP/SSE slice plus CLI/GUI WebSocket HTTP CONNECT routing; explicit CLI/GUI proxy, bypass list and env/system proxy support; SOCKS WebSocket and gRPC routing pending | local forwarding, direct bypass, WebSocket CONNECT relay, SOCKS URL construction and invalid-proxy tests |
