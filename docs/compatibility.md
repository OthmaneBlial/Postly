...
| HTTP(S)/SOCKS proxy routing | working HTTP/SSE slice plus CLI WebSocket HTTP CONNECT routing; explicit CLI/GUI proxy, bypass list and env/system proxy support; SOCKS WebSocket, GUI WebSocket and gRPC routing pending | local forwarding, direct bypass, WebSocket CONNECT relay, SOCKS URL construction and invalid-proxy tests |
