...
Request files are independent merge units. The collection file stores metadata and collection-scoped variables; request files store method, URL, parameters, headers, body, auth, scripts and examples. Saved response examples can carry the HTTP status text, response cookies with common attributes and an optional local mock delay in milliseconds; Postman exports preserve these fields under the corresponding response fields and `x-postly-delay-ms` extension.
