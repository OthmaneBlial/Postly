...
- **Authentication:** Basic, Bearer, API key, OAuth 2.0 Client Credentials,
  Authorization Code + PKCE and Refresh Token exchanges, including variable
...
- **Testing and automation:** response assertions, an opt-in Node.js script
  bridge, tested `pm.*` behavior including bounded `pm.sendRequest` callbacks,
  collection runs, folder selection, iteration data, fail-fast execution,
  pretty/JSON/JUnit reporters and a deterministic local HTTP mock server backed
  by saved response examples.
